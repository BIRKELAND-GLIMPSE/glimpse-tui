"""Today's screens as functions (TERMINAL.md 8): MKT, HM, SLIP, PORT and BOTS.
...
register(Function(
    "MKT", "Markets and ladder", "Glimpse", "Glimpse markets and the ladder", None, legacy="main",
    help="Every live close of the chosen series on the left, with the market's median and 80% band. On the right, the "
         "ladder of price ranges for the selected close: probability, odds net of fees, and your position. "
         "Prices come from the public Glimpse API: the market list every 60 s, the focused book every 5 s. "
         "Keys are the ones the screen has always had: j k move, h l switch pane, v selects a range, b buys after a "
         "confirmation and a server estimate. [ and ] change series. " + _DOWN))
register(Function(
    "HM", "Forecast heatmap", "Glimpse", "the forecast heatmap: every close, every price, one picture", None, legacy="heatmap",
    help="Time runs across, price runs up, and each character is one price cell of one close, denser where the market "
...
register(Function(
    ...
    help="Every Forecast Lab model that runs on hourly candles alone, read against the live market. Tabs sort them by "
